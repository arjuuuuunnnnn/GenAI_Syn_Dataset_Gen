from typing import TypedDict, Annotated, List
from langgraph.graph import StateGraph, END
import google.generativeai as genai
import os
import json
from dotenv import load_dotenv

load_dotenv()

GEMINI_API_KEY = os.getenv("GEMINI_API_KEY")
if not GEMINI_API_KEY:
    raise ValueError("Please set GEMINI_API_KEY in your .env file")

genai.configure(api_key=GEMINI_API_KEY)

try:
    model = genai.GenerativeModel('gemini-2.0-flash')
    test_response = model.generate_content("Hello")
    if not test_response.text:
        raise ConnectionError("Gemini API connection failed - no response text")
except Exception as e:
    raise ConnectionError(f"Failed to initialize Gemini API: {str(e)}")

class AgentState(TypedDict):
    user_query: str
    reasoning: Annotated[List[str], "Chain of thought steps"]
    dataset_description: Annotated[str, "Description of the dataset to be generated"]
    dataset_schema: Annotated[dict, "Schema for the dataset"]
    generated_dataset: Annotated[List[dict], "The generated dataset entries"]
    output: Annotated[str, "The final output to return to the user"]

def extract_json_from_response(response_text):
    """Helper function to extract JSON from Gemini response"""
    try:
        cleaned = response_text.strip()
        if cleaned.startswith('```json'):
            cleaned = cleaned[7:]
        if cleaned.endswith('```'):
            cleaned = cleaned[:-3]
        cleaned = cleaned.strip()
        
        return json.loads(cleaned)
    except json.JSONDecodeError as e:
        print(f"JSON parsing error: {str(e)}")
        print(f"Response text was: {response_text}")
        return None

def parse_user_query(state: AgentState):
    """Parse the user query to understand what dataset they want"""
    try:
        user_query = state["user_query"]
        print(f"\n[DEBUG] Parsing user query: {user_query}")
        
        # CoT Prompt with reasoning steps
        prompt = f"""
        Let's think step by step to analyze this dataset generation request:
        
        1. First, identify the main entities or objects the user wants in their dataset.
        2. Then, extract the specific fields/properties mentioned for these entities.
        3. Identify any constraints or special requirements.
        4. Finally, structure this information into a standardized JSON format.
        
        Return ONLY valid JSON with your analysis:
        {{
          "reasoning": ["step1", "step2"],
          "data_type": "description of data",
          "fields": [{{"name": "field1", "type": "string"}}],
          "constraints": ["constraint1"]
        }}

        User Query: {user_query}
        """
        
        response = model.generate_content(prompt)
        print(f"[DEBUG] Raw API response: {response.text}")
        
        analysis = extract_json_from_response(response.text)
        if not analysis:
            raise ValueError("Failed to parse API response as JSON")
        
        print(f"[DEBUG] Parsed analysis: {analysis}")
        
        # Store both the analysis and the reasoning steps
        reasoning = analysis.pop("reasoning", [f"Initial analysis of user query: {user_query}"])
        
        return {
            "reasoning": reasoning,
            "dataset_description": analysis.get("data_type", user_query),
            "dataset_schema": {
                "fields": analysis.get("fields", [{"name": "field1", "type": "string"}]),
                "constraints": analysis.get("constraints", [])
            }
        }
    except Exception as e:
        error_msg = f"Error parsing query: {str(e)}"
        print(f"[ERROR] {error_msg}")
        return {
            "reasoning": [error_msg],
            "dataset_description": user_query,
            "dataset_schema": {
                "fields": [{"name": "field1", "type": "string"}],
                "constraints": []
            }
        }

def generate_dataset_schema(state: AgentState):
    """Generate a detailed schema based on the initial analysis"""
    try:
        description = state["dataset_description"]
        initial_schema = state["dataset_schema"]
        reasoning = state["reasoning"]
        print(f"\n[DEBUG] Generating schema from: {description}")
        
        # CoT Prompt for schema generation
        prompt = f"""
        Let's develop a comprehensive schema step by step:
        
        1. First, review the dataset description: {description}
        2. Analyze the initial fields: {json.dumps(initial_schema['fields'])}
        3. For each field, determine:
           - Appropriate data type
           - Field description/purpose
           - Example values
           - Any validation rules needed
        4. Consider if additional fields would make the dataset more useful
        5. Organize into a complete schema structure
        
        Return ONLY valid JSON with:
        {{
          "reasoning": ["step1", "step2"],
          "description": "detailed description",
          "fields": [
            {{
              "name": "field_name",
              "type": "data_type",
              "description": "field purpose",
              "example": "example value"
            }}
          ],
          "constraints": ["constraint1"]
        }}
        """
        
        response = model.generate_content(prompt)
        print(f"[DEBUG] Raw schema response: {response.text}")
        
        enhanced_schema = extract_json_from_response(response.text)
        if not enhanced_schema:
            raise ValueError("Failed to parse schema response")
        
        print(f"[DEBUG] Generated schema: {enhanced_schema}")
        
        # Combine previous reasoning with new steps
        new_reasoning = enhanced_schema.pop("reasoning", ["Generated detailed schema"])
        return {
            "reasoning": reasoning + new_reasoning,
            "dataset_schema": enhanced_schema
        }
    except Exception as e:
        error_msg = f"Error generating schema: {str(e)}"
        print(f"[ERROR] {error_msg}")
        return {
            "reasoning": state["reasoning"] + [error_msg],
            "dataset_schema": initial_schema
        }

def generate_dataset_entries(state: AgentState):
    """Generate actual dataset entries based on the schema"""
    try:
        schema = state["dataset_schema"]
        reasoning = state["reasoning"]
        num_entries = 5
        print(f"\n[DEBUG] Generating {num_entries} entries from schema")
        
        # CoT Prompt for data generation
        prompt = f"""
        Let's generate realistic dataset entries step by step:
        
        1. Review the schema: {json.dumps(schema, indent=2)}
        2. For each entry to generate:
           a) Consider field types and constraints
           b) Create realistic values that match field descriptions
           c) Ensure relationships between fields make sense
           d) Maintain variety in the generated values
        3. Validate each entry against the schema before including
        
        Generate {num_entries} entries and return ONLY valid JSON array:
        {{
          "reasoning": ["step1", "step2"],
          "data": [
            {{
              "field1": "value1",
              "field2": "value2"
            }}
          ]
        }}
        """
        
        response = model.generate_content(prompt)
        print(f"[DEBUG] Raw data response: {response.text}")
        
        result = extract_json_from_response(response.text)
        if not result or not isinstance(result.get("data", []), list):
            raise ValueError("Invalid dataset format")
        
        print(f"[DEBUG] First entry: {result['data'][0] if result['data'] else 'empty'}")
        
        # Combine reasoning and get the dataset
        new_reasoning = result.get("reasoning", [f"Generated {num_entries} dataset entries"])
        return {
            "reasoning": reasoning + new_reasoning,
            "generated_dataset": result["data"]
        }
    except Exception as e:
        error_msg = f"Error generating entries: {str(e)}"
        print(f"[ERROR] {error_msg}")
        return {
            "reasoning": state["reasoning"] + [error_msg],
            "generated_dataset": []
        }

def validate_dataset(state: AgentState):
    """Validate the generated dataset against the schema"""
    try:
        schema = state["dataset_schema"]
        dataset = state["generated_dataset"]
        reasoning = state["reasoning"]
        if not dataset:
            return {"generated_dataset": []}
        
        print(f"\n[DEBUG] Validating {len(dataset)} entries")
        
        # CoT Prompt for validation
        prompt = f"""
        Let's validate the dataset step by step:
        
        1. Review the schema requirements: {json.dumps(schema, indent=2)}
        2. For each entry in the dataset:
           a) Check all required fields are present
           b) Verify data types match schema
           c) Validate against constraints
           d) Check for logical consistency
        3. Identify any patterns in validation issues
        4. If issues found, suggest corrections
        
        Return ONLY valid JSON with:
        {{
          "reasoning": ["step1", "step2"],
          "is_valid": true|false,
          "issues": ["issue1"],
          "validated_dataset": [...]  // Only if corrections needed
        }}

        Dataset Sample: {json.dumps(dataset[:3], indent=2)}...
        """
        
        response = model.generate_content(prompt)
        print(f"[DEBUG] Raw validation response: {response.text}")
        
        validation = extract_json_from_response(response.text)
        if not validation:
            return {
                "reasoning": reasoning + ["Validation skipped - no result"],
                "generated_dataset": dataset
            }
        
        # Combine reasoning and process validation results
        new_reasoning = validation.pop("reasoning", ["Performed dataset validation"])
        final_reasoning = reasoning + new_reasoning
        
        if not validation.get("is_valid", False):
            issues = validation.get("issues", [])
            print(f"[WARNING] Validation issues: {issues}")
            final_reasoning.append(f"Found issues: {', '.join(issues)}")
            
            if "validated_dataset" in validation:
                return {
                    "reasoning": final_reasoning,
                    "generated_dataset": validation["validated_dataset"]
                }
        
        return {
            "reasoning": final_reasoning,
            "generated_dataset": dataset
        }
    except Exception as e:
        error_msg = f"Error during validation: {str(e)}"
        print(f"[ERROR] {error_msg}")
        return {
            "reasoning": state["reasoning"] + [error_msg],
            "generated_dataset": dataset
        }

def format_output(state: AgentState):
    """Format the final output for the user"""
    try:
        schema = state["dataset_schema"]
        dataset = state["generated_dataset"]
        reasoning = state["reasoning"]
        
        # Build the output with reasoning included
        output = f"""
        # Generated Dataset
        
        ## Reasoning Steps
        {"\n".join(f"- {step}" for step in reasoning)}
        
        ## Schema Description
        {schema.get('description', 'No description available')}
        
        ## Fields
        """
        
        for field in schema.get("fields", []):
            output += f"\n- **{field.get('name', '')}**: {field.get('description', '')} ({field.get('type', '')})"
            if "example" in field:
                output += f" - Example: `{field['example']}`"
        
        output += "\n\n## Dataset Preview\n```json\n"
        output += json.dumps(dataset[:3], indent=2) if dataset else "[]"
        output += "\n```"
        
        output += f"\n\nTotal entries generated: {len(dataset) if dataset else 0}"
        
        # Save to file with reasoning included
        try:
            with open("generated_dataset.json", "w") as f:
                json.dump({
                    "reasoning": reasoning,
                    "schema": schema,
                    "data": dataset if dataset else []
                }, f, indent=2)
            print("[DEBUG] Dataset saved to file")
        except Exception as e:
            print(f"[ERROR] saving dataset: {str(e)}")
        
        return {"output": output}
    except Exception as e:
        print(f"[ERROR] in format_output: {str(e)}")
        return {"output": "Error generating output"}

# Create and configure the workflow
workflow = StateGraph(AgentState)
workflow.add_node("parse_query", parse_user_query)
workflow.add_node("generate_schema", generate_dataset_schema)
workflow.add_node("generate_entries", generate_dataset_entries)
workflow.add_node("validate_data", validate_dataset)
workflow.add_node("format_output", format_output)

workflow.add_edge("parse_query", "generate_schema")
workflow.add_edge("generate_schema", "generate_entries")
workflow.add_edge("generate_entries", "validate_data")
workflow.add_edge("validate_data", "format_output")
workflow.add_edge("format_output", END)
workflow.set_entry_point("parse_query")

agent = workflow.compile()

if __name__ == "__main__":
    print("Starting dataset generation agent...")
    user_query = "I need a dataset of fictional books with title, author, genre, and year"
    
    try:
        print(f"\nProcessing query: '{user_query}'")
        result = agent.invoke({"user_query": user_query, "reasoning": []})
        
        if not result or "output" not in result:
            raise ValueError("Agent didn't return expected output format")
        
        print("\n=== FINAL OUTPUT ===")
        print(result["output"])
        print("\nDataset saved to 'generated_dataset.json'")
    except Exception as e:
        print(f"\n[CRITICAL ERROR] Agent failed: {str(e)}")
        print("Possible causes:")
        print("- Invalid Gemini API key (even if you think it's valid)")
        print("- Network connectivity issues")
        print("- Google API service outage")
        print("- Bug in the code (unlikely but possible)")
        print("\nCheck the debug output above for more clues.")
